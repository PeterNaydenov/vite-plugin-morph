...
_Import this file in your main entry point or application root._
...
_The plugin will process these into `dist/themes/default.css` and `dist/themes/dark.css`._
...
<div class="{{ type : getAlertClass }}">{{ message }}</div>
...
```javascript
import { themeRuntime } from '@peter.naydenov/vite-plugin-morph/browser';

// Initialize the runtime helper
...
const themes = runtime.list(); // ['default', 'dark', 'ocean']

// Switch to a different theme
if (themes.includes('dark')) {
  runtime.set('dark'); // Switches to 'dark' theme
}

// Get reset to default
const defaultTheme = runtime.getDefault();
...
If you prefer `getThemeRuntime` for manual control without auto-discovery:
...
// Manual initialization required if not using themeRuntime()
const myThemes = new Map([
    ['light', { /* ... */ }],
    ['dark', { /* ... */ }]
]);

runtime.initialize(myThemes);
```
