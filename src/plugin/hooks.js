...
function createErrorReturn(result, extraProps = {}) {
  return {
...
function createSuccessResult(result, extraProps = {}) {
  const safeCode =
...
function createErrorCatchReturn(err, id) {
...
export async function transformHook(code, id, options = {}) {
  ...
  try {
    ...
    if (result.errors && result.errors.length > 0) {
      return createErrorReturn(result);
    }

    const transformResult = createSuccessResult(result);
    info(`Successfully transformed ${id} in ${result.processingTime}ms`);
    return transformResult;
  } catch (err) {
    return createErrorCatchReturn(err, id);
  }
}
